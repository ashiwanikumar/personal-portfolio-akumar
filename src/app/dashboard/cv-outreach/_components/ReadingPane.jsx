...
function ReplyStatus({ message }) {
	if (message.bounced) {
		return (
			<div className="rounded-md border border-[#FB7185]/45 border-l-[3px] border-l-[#FB7185] bg-[#2A1218] px-4 py-3.5">
	...
	if (message.replied) {
		return (
			<div className="rounded-md border border-[#34D399]/35 border-l-[3px] border-l-[#34D399] bg-[#0B2A20] px-4 py-3.5">
				...
				{message.firstReplySnippet && (
					<p className="mt-3 border-l-2 border-[#363C47] pl-3 text-[13px] leading-5 text-[#A1A1AA]">
						{message.firstReplySnippet}
					</p>
				)}
			</div>
	...
	return (
		<div className="rounded-md border border-[#4E3A0D] border-l-[3px] border-l-[#FBBF24] bg-[#251B07] px-4 py-3.5">
	...
	return (
		...
		>
			...
			<div className="min-w-0 flex-1">
				<p className="truncate text-[13px] text-[#F4F4F5]">{attachment.filename}</p>
...
export default function ReadingPane({ message, related, onBack, onToggleStar, onOpenRelated }) {
	...
	return (
		<div className="nx-rise flex min-h-0 flex-1 flex-col">
			{/* Toolbar */}
			<div className="flex h-12 shrink-0 items-center gap-1 border-b border-[#21252D] px-2">
				<IconButton label="Back to list" onClick={onBack}>
					<Icon path={MailIcons.arrowBack} className="h-[18px] w-[18px]" />
				</IconButton>
				<div className="mx-1 h-5 w-px bg-[#21252D]" />
			...
			<div className="nx-scroll-thin min-h-0 flex-1 overflow-y-auto px-4 py-4 sm:px-8">
				<div className="mb-6 flex items-start gap-3">
					<h2 className="nx-display flex-1 text-[20px] leading-7 text-[#F4F4F5] sm:text-[22px]">
						{message.subject}
					</h2>
					<span className="nx-mono mt-1 inline-flex shrink-0 items-center gap-1 rounded border border-[#21252D] bg-[#07080A] px-2 py-0.5 text-[10px] uppercase tracking-wide text-[#A1A1AA]">
				...
				<div className="flex items-start gap-3">
					...
					<div className="min-w-0 flex-1">
						<div className="flex flex-wrap items-baseline gap-x-2">
							<span className="text-[14px] font-semibold text-[#F4F4F5]">me</span>
							<span className="nx-mono text-[11px] text-[#70747E]">
								&lt;{message.fromEmail || "you"}&gt;
							</span>
							<span className="nx-mono ml-auto text-[11px] tabular-nums text-[#70747E]">
						...
						<button
							...
							className="mt-0.5 flex items-center gap-1 text-[12px] text-[#70747E] transition-colors hover:text-[#F4F4F5]"
						>
						...
						{detailsOpen && (
							<dl className="nx-rise mt-2 grid grid-cols-[64px_1fr] gap-x-3 gap-y-1 rounded-md border border-[#21252D] bg-[#12151A] p-3 text-[12px]">
								<dt className="nx-eyebrow pt-0.5">from</dt>
								<dd className="nx-mono text-[11px] text-[#F4F4F5]">{message.fromEmail || "—"}</dd>
								<dt className="nx-eyebrow pt-0.5">to</dt>
								<dd className="nx-mono break-all text-[11px] text-[#F4F4F5]">
									{(message.to || []).join(", ") || "—"}
								</dd>
								{message.cc?.length > 0 && (
									<>
										<dt className="nx-eyebrow pt-0.5">cc</dt>
										<dd className="nx-mono break-all text-[11px] text-[#F4F4F5]">{message.cc.join(", ")}</dd>
									</>
								)}
								{message.bcc?.length > 0 && (
									<>
										<dt className="nx-eyebrow pt-0.5">bcc</dt>
										<dd className="nx-mono break-all text-[11px] text-[#F4F4F5]">{message.bcc.join(", ")}</dd>
									</>
								)}
								<dt className="nx-eyebrow pt-0.5">date</dt>
								<dd className="text-[#F4F4F5]">{formatFullDate(message.sentAt)}</dd>
								<dt className="nx-eyebrow pt-0.5">company</dt>
								<dd className="text-[#F4F4F5]">
									{message.companyName || "—"}{" "}
									<span className="nx-mono text-[11px] text-[#70747E]">
						...
						{/* Body */}
						<div className="mt-5 text-[14px] leading-6 text-[#F4F4F5]">
							{loadingBody ? (
								<div className="space-y-2">
									{[90, 75, 82, 60].map((w, i) => (
										<div key={i} className="h-3 animate-pulse rounded bg-[#21252D]" style={{ width: `${w}%` }} />
									))}
								</div>
							) : bodyError ? (
								<div className="rounded-md border border-[#FB7185]/45 border-l-[3px] border-l-[#FB7185] bg-[#2A1218] p-4">
									<p className="text-[13px] text-[#FB7185]">{bodyError}</p>
									<p className="mt-1 text-[12px] text-[#70747E]">Showing the stored preview instead.</p>
									<p className="mt-3 text-[13px] text-[#A1A1AA]">{message.snippet}</p>
								</div>
							) : body?.format === "html" ? (
								<div
								...
								<p className="text-[#A1A1AA]">{message.snippet}</p>
							)}
						</div>

						{/* Attachments */}
						{message.attachments?.length > 0 && (
							<div className="mt-8 border-t border-[#21252D] pt-4">
						...
						{related?.length > 0 && (
							<div className="mt-6">
								...
								<div className="nx-stagger divide-y divide-[#21252D] overflow-hidden rounded-md border border-[#21252D]">
									{related.map((item) => (
										<button
											key={item.gmailMessageId}
											type="button"
											onClick={() => onOpenRelated(item.gmailMessageId)}
											className="flex w-full items-center gap-3 bg-[#12151A] px-3 py-2 text-left transition-colors hover:bg-[#0B0D10]"
										>
											<Icon
												path={item.replied ? MailIcons.reply : MailIcons.schedule}
												className={`h-4 w-4 shrink-0 ${item.replied ? "text-[#34D399]" : "text-[#FBBF24]"}`}
											/>
											<span className="min-w-0 flex-1 truncate text-[13px] text-[#F4F4F5]">{item.subject}</span>
